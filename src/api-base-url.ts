...
export function normalizeTaskboiApiBaseUrl(value: string | undefined): string {
  if (value === undefined || value.trim() === "") {
    throw new Error("TASKBOI_API_BASE_URL environment variable is required");
  }
  if (value !== value.trim()) {
    throw new Error("TASKBOI_API_BASE_URL must not contain surrounding whitespace");
  }
  let url: URL;
  try {
    url = new URL(value);
  } catch {
    throw new Error("TASKBOI_API_BASE_URL must be an absolute HTTPS URL");
  }
...
export function apiRequestUrl(baseUrl: string, path: string): string {
  ...
  for (const segment of pathname.split("/")) {
    ...
    try {
      ...
      throw new Error("Taskboi API request path contains invalid encoding");
    }
